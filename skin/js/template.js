...
Switcher = function(){
	...
	this.startSwitcher = function() {
		...
		if (s.pagebt == 1 && s.thisSwitcher.find(".bigpagination")[0] != undefined) {
...
/* video js */
function videojs(){
	$('#item-479').click(function(){
		if(!$('.videodiv').isExist()){
			alert('视频正在添加中，敬请期待！');
		}else{			
			var b = $('.videodiv').html();
			var img = $('<div class="alertvideo">'+b+'</div>');
			// var close=$('.videodivclose');
			img.setLightBox({layoverBg:'#777',layoverOpa:'0.7',alertdivBg:'transparent',closeBut:'.videodivclose'});
		}
	});	
}
function changeheight(){
	if(!$('.innercontainer').isExist())return;
		
	if($('img').isExist()){
	$('img').load(function(){	
		if($('.protab').isExist()){
			var proheight=$('.propan').first().height();
			$('.propan').each(function(i){
				if($(this).height()>proheight){
					proheight=$(this).height();
				}
			});
			$('.propan').height(proheight);
		}
		var leftheight=$('#left-menu').height();
		var rightheight=$('#component-content').height();
		if((leftheight-20)>rightheight){
			//alert(leftheight-20+'rightheight:'+rightheight);
				$('#component-content').height(leftheight-20);
			}else{
				$('#left-menu').height(rightheight+20);
			}
	});
	}else{
		if($('.protab').isExist()){
...
function leftmenu(){
if(!jQuery('#left-menu').isExist())return;	
	$('#left-menu .menu li ul li').each(function(){
		if($(this).hasClass('active')){
		$(this).parent().parent().addClass('downlicss');
	}
	});
	
}
function showtab0(){
	if(!jQuery('#tabdiv').isExist())return;	
	var buts2=['.tab'];
	var tabs2=['.pandiv'];
	$(buts2.join()).eq(0).addClass('curtab');
	function backfn2(i){
		$(buts2.join()).removeClass('curtab');
		$(buts2.join()).eq(i).addClass('curtab');
	}
	$.fn.butShowTab(buts2,tabs2,-1,'click',backfn2,'',0);
}
function showtab(){
	if(!jQuery('.product-desc-tip').isExist())return;
	var buts1=['.protab'];
	var tabs1=['.propan'];
	$(buts1.join()).eq(0).addClass('curtab');
	function backfn1(i){
		$(buts1.join()).removeClass('curtab');
		$(buts1.join()).eq(i).addClass('curtab');
	}
	$.fn.butShowTab(buts1,tabs1,-1,'click',backfn1,'',0);
