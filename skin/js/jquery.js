	...
	*/	
	jQuery.fn.setLightBox=function(options){
		...
		if(jQuery(window).height()<jQuery('body').outerHeight(true)){		
				...
				layerdiv.height(jQuery(window).height());
			}
			...
			if(!options.canNotDrag){
				...
				if(alertdiv.find('img').length!=0){
					...
					if(options.closeBut){
						...
						dragbodys.drag({dragbody: alertdiv,opacity: '0.8',preventEvent:true,mouseupFn:function(){if(!draged){jQuery.fn.closeLightBox();}}});
					}
				}else{
					if(options.closeBut){
						dragbodys.drag({dragbody: alertdiv,opacity: '0.8'});
					}else{
						dragbodys.drag({dragbody: alertdiv,opacity: '0.8',mouseupFn:function(){if(!draged){jQuery.fn.closeLightBox();}}});
					}
				}
			}
